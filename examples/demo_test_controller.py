...
if __name__ == "__main__":
    ...
    target_traj_2 = np.linspace(
        target_traj_1[-1, :],
        np.asarray([goal_pos_2[0], goal_pos_2[1], goal_pos_2[2]]),
        20,
    )
    goal_pos_3 = target_traj_2[-1, :].copy()
    goal_pos_3[1] += 0.4
    target_traj_3 = np.linspace(target_traj_2[-1, :], goal_pos_3, 200)
    target_traj = np.vstack([target_traj_1, wait_traj, target_traj_2, target_traj_3])
    # --------------------------------------
    ...
    while i < target_traj.shape[0]:
        ...
        if i == 130:  # activate force control when the robot is near the table
            ...
            ctrl.set_goal(
                target_traj[i, :], target_ori, goal_force=[0, 0, -25]
