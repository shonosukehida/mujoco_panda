...
def quat2Vel(quat):
    ...
    """
    res = np.zeros(3)
    mujoco_py.functions.mju_quat2Vel(res, quat, 1.0)
    return res
...
def subQuat(qb, qa):
    ...
    mujoco_py.functions.mju_quat2Vel(res, q_diff, 1.0)
...
def quatIntegrate(q, v, dt=1.0):
    res = q.copy()
    mujoco_py.functions.mju_quatIntegrate(res, v, 1.0)
    return res
